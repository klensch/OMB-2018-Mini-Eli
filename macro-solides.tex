...
\def\pasPrismereg[#1]{
	\setkeys[PAS]{prismereg}{#1}
		...
		\foreach \i in {1,...,\cote}
		{
			...
			\IfStrEq{\i}{1}{\global\def\xib{\Mx}\global\def\yib{\My}}{}
		...
		\ifPAS@prismereg@legende
			\IfStrEq{\cmdPAS@prismereg@incolor}{white}{\def\couleur{black}}{\def\couleur{\cmdPAS@prismereg@bordercolor}}
			% Bases
			...
			\draw[<-,>=stealth,\couleur] (.5,-0.1) to [bend right=30] (\x,\y+0.5) node[right,\couleur] {\footnotesize\text{base $B$ (polygonale)}};
			\draw[<-,>=stealth,\couleur] (.5,0.2+\cmdPAS@prismereg@hauteur) to [bend left=30] (\x,\y+\cmdPAS@prismereg@hauteur+2.5) node[right,\couleur] {\footnotesize\text{base $B$ (polygonale)}};
			% Faces latérales
			\pgfmathparse{\cmdPAS@prismereg@rayon*cos(\rotat)}\let\Mx\pgfmathresult
			...
			\draw[<-,>=stealth,\couleur] (G) to [bend left=30] ($(G)+(1.,0.75)$) node[right,\couleur] {\begin{minipage}{3cm}\footnotesize\text{face lat\'erale}\end{minipage}};
			% Hauteur
			%\draw[\cmdPAS@prismereg@bordercolor,dotted] (T\cotes) -- ($(T\cotes)-(4,0)$);
			%\draw[\cmdPAS@prismereg@bordercolor,dotted] (M) -- ($(M)-(4,0)$);
			\draw[|<->|,\cmdPAS@prismereg@bordercolor,>=stealth'] ($(T\cotes)+(0.3,0)$) -- ($(M)+(0.3,0)$) node[midway,right,\cmdPAS@prismereg@bordercolor] {\footnotesize{$h$}};
			
			\tkzDrawPoint[](0,0)
			\tkzDrawPoint[](0,\cmdPAS@prismereg@hauteur)
		\fi
		\ifPAS@prismereg@axe
			...
			\draw[\cmdPAS@prismereg@axecolor] (0,-\cmdPAS@prismereg@incl) -- (0,-\cmdPAS@prismereg@incl-0.6);
			\ifPAS@prismereg@legende
				\draw[<-,>=stealth',\cmdPAS@prismereg@axecolor] (-0.2,\cmdPAS@prismereg@hauteur+.75) to [bend left=20] (-\cmdPAS@prismereg@rayon/2-0,\cmdPAS@prismereg@hauteur+1.2) node[above,\cmdPAS@prismereg@axecolor] {\footnotesize\text{axe de rotation}};
			\fi
		\fi
		\ifPAS@prismereg@name
			\foreach \i in {1,2,...,\cotes}
...
\def\pasPyramreg[#1]{
	\setkeys[PAS]{pyramreg}{#1}
		...
		\ifPAS@pyramreg@legende
			...
			\draw[<-,>=stealth,\cmdPAS@pyramreg@incolor] (.5,-.5) to [bend right=30] (\x,\y+0.4) node[right,\cmdPAS@pyramreg@bordercolor] {\footnotesize\text{base $B$ (polygonale)}};
		\fi
		\ifPAS@pyramreg@name
		...
		\ifPAS@pyramreg@axe
			...
			\draw[\cmdPAS@pyramreg@axecolor] (0,-\cmdPAS@pyramreg@incl) -- (0,-\cmdPAS@pyramreg@incl-0.6);
			\ifPAS@pyramreg@legende
				\draw[<-,>=stealth',\cmdPAS@pyramreg@axecolor] (-0.2,\cmdPAS@pyramreg@hauteur+.4) to [bend left=20] (-\cmdPAS@pyramreg@rayon/2+0,\cmdPAS@pyramreg@hauteur+0.8) node[above,\cmdPAS@pyramreg@axecolor] {\footnotesize\text{axe de rotation}};
			\fi
		\fi
		\ifPAS@pyramreg@legende
			...
			\draw[<-,>=stealth,\cmdPAS@pyramreg@bordercolor] (\Nx/2,\y) to [bend left=30] (\cmdPAS@pyramreg@rayon+.5,\y+0.7) node[right,\cmdPAS@pyramreg@bordercolor] {\footnotesize\text{ar\^ete lat\'erale}};
			\draw[<-,>=stealth,\cmdPAS@pyramreg@bordercolor] (0,\cmdPAS@pyramreg@hauteur) to [bend left=10] (\cmdPAS@pyramreg@rayon+.5,\cmdPAS@pyramreg@hauteur+0.5) node[right,\cmdPAS@pyramreg@bordercolor] {\footnotesize\text{apex}};
			...
			\draw[<-,>=stealth,\cmdPAS@pyramreg@bordercolor] (\Nx/2,.5) to [bend right=30] (\cmdPAS@pyramreg@rayon+.5,-0.8) node[right,\cmdPAS@pyramreg@bordercolor] {\footnotesize\text{face lat\'erale}};
...
\def\pasBoule[#1]{
	...
	\begin{scope}[scale=\cmdPAS@boule@scale]
		...
		\ifPAS@boule@equateurlegende
			\draw[<-,>=stealth',\cmdPAS@boule@bordercolor] (1.8,-.3) to [bend right=30] (2.5,-0.4) node[right,\cmdPAS@boule@bordercolor] {\footnotesize\text{équateur}};
		\fi
		\ifPAS@boule@exemplecoord
			...
			\draw[dashed,\cmdPAS@boule@bordercolor] (0,0) -- (1.28,-0.38) node[midway,above] {\footnotesize{$r$}};
...
\def\pasCone[#1]{
	\setkeys[PAS]{cone}{#1}
		...
		\draw[\cmdPAS@cone@bordercolor] (0,0) arc (180:0:\cmdPAS@cone@rayon cm and \petitaxe cm);
		\ifPAS@cone@axe
			...
			\ifPAS@cone@legende
				\draw[\cmdPAS@cone@axecolor,<-,>=stealth'] (\cmdPAS@cone@rayon+0.2,\cmdPAS@cone@hauteur+.5) to [bend right=30] (\cmdPAS@cone@rayon+1,\cmdPAS@cone@hauteur+1) node[above,\cmdPAS@cone@axecolor] {\footnotesize\text{axe de r\'evolution}};
			\fi		
		\fi
		...
		\draw[black] (0,0) -- (\cmdPAS@cone@rayon,\cmdPAS@cone@hauteur) -- (\grandaxe,0) arc (0:-180:\cmdPAS@cone@rayon cm and \petitaxe cm);
		\ifPAS@cone@name
		...
		\ifPAS@cone@legende
			...
			\draw[<-,>=stealth',\cmdPAS@cone@bordercolor] (\x,\y) to [bend right=30] (2*\cmdPAS@cone@rayon+0.1,-0.8) node[\cmdPAS@cone@bordercolor,right] {\footnotesize\text{g\'en\'eratrice}};
			\draw[black,<-,>=stealth'] (\cmdPAS@cone@rayon,\cmdPAS@cone@hauteur) to [bend right=20] (\cmdPAS@cone@rayon/2,\cmdPAS@cone@hauteur+0.5) node[black,left] {\footnotesize\text{apex}};
			\draw[black,<-,>=stealth'] (\cmdPAS@cone@rayon-0.8,0) to [bend right=30] (\cmdPAS@cone@rayon/2-0.3,-\petitaxe-.5) node[black,below] {\footnotesize\text{base (disque)}};
			
			\draw[black,<-,>=stealth'] (\cmdPAS@cone@rayon-1,2) to [bend left=30] (\cmdPAS@cone@rayon/2-1.2,-\petitaxe+3.5) node[black,above] {\footnotesize\text{face latérale}};
			
			\draw[|<->|,\cmdPAS@cone@bordercolor,>=stealth'] ($(\cmdPAS@cone@rayon,\cmdPAS@cone@hauteur)+(3.3,0)$) -- ($(\cmdPAS@cone@rayon,0)+(3.3,0)$) node[midway,right,\cmdPAS@cone@bordercolor] {\footnotesize{$h$}};
			
			\draw[dashed,\cmdPAS@cone@bordercolor] ($(\cmdPAS@cone@rayon,0)+(0,0)$) -- ($(\cmdPAS@cone@rayon,0)+(3,0)$) node[midway,above,\cmdPAS@cone@bordercolor] {\footnotesize{$r$}};
			\tkzDrawPoint(\cmdPAS@cone@rayon,0)
		\fi
}
	...
	\setkeys[PAS]{cylindre}{#1}
		...
		\ifPAS@cylindre@rectgener
			...
			\ifPAS@cylindre@legende
				\draw[\cmdPAS@cylindre@bordercolor,<-,>=stealth'] (.75*\x-0.1,.75*\y+.3) to [bend left=35] (1.3*\x+0.5-1.7,2*\y-0.4) node[left,\cmdPAS@cylindre@bordercolor] {\footnotesize\text{rectangle g\'en\'erateur}};
			\fi
		\fi			
		\draw[\cmdPAS@cylindre@bordercolor] (\cmdPAS@cylindre@rayon,0) arc (0:180:\cmdPAS@cylindre@rayon cm and \petitaxe cm);
		\ifPAS@cylindre@axe
			...
			\draw[\cmdPAS@cylindre@axecolor] (0,-\petitaxe) -- (0,-\petitaxe-0.8);
			\ifPAS@cylindre@legende
				\draw[\cmdPAS@cylindre@axecolor,<-,>=stealth'] (0.1,\cmdPAS@cylindre@hauteur+.6) to [bend right=30] (0.6,\cmdPAS@cylindre@hauteur+1) node[above,\cmdPAS@cylindre@axecolor] {\footnotesize\text{axe de r\'evolution}};
			\fi		
		\fi
		...
		\ifPAS@cylindre@legende		
			...
			\draw[black,<-,>=stealth',\cmdPAS@cylindre@bordercolor] (-0.5*\cmdPAS@cylindre@rayon,0.5*\cmdPAS@cylindre@hauteur+.2) to [bend right=25] (-1.2*\cmdPAS@cylindre@rayon,0.5*\cmdPAS@cylindre@hauteur+.7) node[\cmdPAS@cylindre@bordercolor,left] {\footnotesize\text{face lat\'erale}};
			
			\draw[black,<-,>=stealth',\cmdPAS@cylindre@bordercolor] (-0.5*\cmdPAS@cylindre@rayon,1.*\cmdPAS@cylindre@hauteur) to [bend right=25] (-1.2*\cmdPAS@cylindre@rayon,1.*\cmdPAS@cylindre@hauteur+.3) node[\cmdPAS@cylindre@bordercolor,left] {\footnotesize\text{base (disque)}};
			
			\draw[black,<-,>=stealth',\cmdPAS@cylindre@bordercolor] (-0.5*\cmdPAS@cylindre@rayon,0.03*\cmdPAS@cylindre@hauteur) to [bend right=25] (-1.2*\cmdPAS@cylindre@rayon,0.05*\cmdPAS@cylindre@hauteur+.3) node[\cmdPAS@cylindre@bordercolor,left] {\footnotesize\text{base (disque)}};
			
		\fi	
		\draw[dashed,\cmdPAS@cylindre@bordercolor] (0,0) -- (-\cmdPAS@cylindre@rayon,0) node[midway,below,\cmdPAS@cylindre@bordercolor] {\footnotesize{$r$}};
		\tkzDrawPoint(0,0)
...
\def\pasCube[#1]{
	...
	\begin{scope}[scale=\cmdPAS@cube@scale]
		\shade[draw=\cmdPAS@cube@bordercolor,shading=ball, ball color=\cmdPAS@cube@incolor,opacity=\cmdPAS@cube@coefopaq] (0,0) -- (1,0) -- (1,1) -- (0,1) -- cycle;
		%below
		\draw[\cmdPAS@cube@bordercolor](0,0) -- (1,0) node [midway, below=2pt] {\footnotesize{$a$}} -- (1,1) -- (0,1) -- cycle node [midway, left=2pt] {\footnotesize{$c$}};
		\pgfmathparse{\cmdPAS@cube@prof*cos(\cmdPAS@cube@angle)/2}\let\x\pgfmathresult
		\pgfmathparse{\cmdPAS@cube@prof*sin(\cmdPAS@cube@angle)/2}\let\y\pgfmathresult
		\pgfmathparse{1+\x}\let\xx\pgfmathresult
		\pgfmathparse{1+\y}\let\yy\pgfmathresult
		...
		\shade[draw=\cmdPAS@cube@bordercolor,shading=ball, ball color=\cmdPAS@cube@incolor,opacity=\cmdPAS@cube@coefopaq] (0,1) -- (\x,\yy) -- (\xx,\yy) -- (1,1) -- cycle;
		%below
		\draw[\cmdPAS@cube@bordercolor](0,1) -- (\x,\yy) -- (\xx,\yy) -- (1,1) -- cycle;
		\filldraw[draw=\cmdPAS@cube@bordercolor,shading=ball, ball color=\cmdPAS@cube@incolor,opacity=\cmdPAS@cube@coefopaq] (1,0) -- (1,1) -- (\xx,\yy) -- (\xx,\y) -- cycle;
		\draw[\cmdPAS@cube@bordercolor] (0,0) -- (\x,\y) -- (\xx,\y);
		\draw[\cmdPAS@cube@bordercolor] (\x,\y) -- (\x,\yy);
		%below
		\draw[\cmdPAS@cube@bordercolor](1,0) -- (1,1) -- (\xx,\yy) -- (\xx,\y) -- cycle node [midway,below right=0.75pt] {\footnotesize{$b$}};
		\ifPAS@cube@name
...
% 			\node[above,\cmdPAS@cube@bordercolor] at (\x,\yy) {H};
		...
		\ifPAS@cube@legende
			\pgfmathparse{(\xx-1)/2+1}\let\xxx\pgfmathresult
			\pgfmathparse{(\yy-1)/2+1}\let\yyy\pgfmathresult			
			\draw[<-,>=stealth',\cmdPAS@cube@bordercolor] (.5,0) to [bend right=30] (2,0) node[right,\cmdPAS@cube@bordercolor] {\footnotesize\text{ar\^ete}};
			\IfStrEq{\cmdPAS@cube@incolor}{white}{\def\c{black}}{\def\c{\cmdPAS@cube@bordercolor}}
			\draw[<-,>=stealth',\c] (\xxx,.5) to [bend left=30] ($(\xxx,.5)+(1,0.25)$) node[right,\c] {\footnotesize\text{face}};
			\fill[\cmdPAS@cube@bordercolor] (1,1) circle (0.01cm);
			\draw[<-,>=stealth',\cmdPAS@cube@bordercolor] (1,1) to [bend left=45] ($(\xx,\yy)+(0.25,0.25)$) node[right,\cmdPAS@cube@bordercolor] {\footnotesize\text{sommet}};
		\fi
	\end{scope}
}